...
class MarketMetricsAnalyzer:
    ...
    def __init__(self, helius_client=None, dexscreener_client=None):
        ...
        self.cache_ttl = int(Config.get("MARKET_METRICS_CACHE_TTL", 300))  # 5 minutos
        self.trending_ttl = int(Config.get("TRENDING_DATA_TTL", 1800))  # 30 minutos
        self.holder_growth_ttl = int(Config.get("HOLDER_GROWTH_TTL", 3600))  # 1 hora
    ...
    async def get_holder_growth(self, token):
        ...
        # Si el caché no es válido, consultamos nuevos datos
        holder_count = 0
        source = "unknown"
        
        # Intentar obtener datos de Helius primero
        ...
        # Si no hay datos de Helius, intentar consulta directa
        if holder_count == 0:
            try:
                ...
                async with session.post(url, json=data, timeout=10) as response:
                    if response.status == 200:
                        ...
                        if token_metadata and len(token_metadata) > 0:
                            token_info = token_metadata[0]
                            if "offChainData" in token_info and "holderCount" in token_info["offChainData"]:
        ...
        # Si aún no hay datos, intentar con DexScreener
        ...
        # Si no conseguimos datos de holders, intentar estimar con fórmula aproximada
        if holder_count == 0:
            try:
                if self.helius_client:
                    if hasattr(self.helius_client, 'get_token_data'):
        ...
        # Inicializar datos de holders si es necesario
        ...
        # Si tenemos un recuento válido, actualizar datos
        if holder_count > 0:
            holder_entry = self.holder_data[token]
            ...
            is_growing = holder_entry["growth_rate_1h"] > 1
            ...
            if len(timestamps) >= 3:
                ...
                if counts[0] > 0 and counts[recent_idx] > 0:
    ...
    async def analyze_liquidity(self, token):
        ...
        # 4. Datos de Helius si no hay otra información
        ...
        # Placeholder para liquidez bloqueada
        is_locked = False
        for dex, data in dex_data.items():
    ...
    async def _get_raydium_liquidity(self, token):
        """
        Obtiene datos de liquidez de Raydium.
    ...
    async def _get_jupiter_liquidity(self, token):
        """
        Obtiene datos de liquidez de Jupiter.
    ...
    async def check_trending_status(self, token):
        ...
        result = {
            "is_trending": False,
            "trending_platforms": [],
            "trend_score": 0,  # 0-1 score
            "discovery_potential": 0  # 0-1 score
        }
        ...
        # Consultar trending en DexScreener
        trending_platforms = []
        ...
        # Usar DexScreener como proxy para Birdeye
        if dexscreener_trending:
        ...
        # Calcular potencial de descubrimiento basándose en liquidez y crecimiento de holders
