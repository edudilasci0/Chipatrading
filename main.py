...
# Configuración de logging: se registran mensajes en app.log y en la salida estándar.
logging.basicConfig(
    level=logging.INFO,
...
def process_feed_data():
    """
    Consulta el feed de Cielo, imprime la respuesta completa para verificar su estructura,
    y procesa las transacciones si se encuentran.
    """
    ...
    try:
        ...
        # Imprime y registra la respuesta completa del feed para depuración
        logging.info("Respuesta completa del feed: %s", data)
        print("Respuesta completa del feed:", data)
        
        if data:
            # Ajusta la clave 'transactions' si la estructura real es diferente
            transactions = data.get("transactions", [])
            if not transactions:
                logging.error("No se encontraron transacciones en la respuesta.")
            else:
...
def main():
    ...
    # Inicializa el tracker (si se utiliza para gestionar wallets)
