...
async def cleanup_discoveries_periodically(scalper_monitor, interval=3600):
    """Tarea periódica para limpiar descubrimientos antiguos."""
    while True:
        try:
            # (Si deseas agregar lógica de limpieza, puedes hacerlo aquí)
            await asyncio.sleep(interval)
        except Exception as e:
            logger.error(f"Error en cleanup_discoveries_periodically: {e}")
...
async def main():
    ...
    try:
        ...
        # Inicializar SignalLogic y agregar referencia a WalletTracker
        ...
        # Iniciar healthcheck
        from signal_logic import add_healthcheck
        ...
        on_message_callback = lambda message: cielo_message_handler(wallet_tracker, scoring_system, signal_logic, scalper_monitor)(message)
    ...
    class BotHealthCheck:
        ...
        def generate_health_report(self):
            ...
            try:
                ...
                self.last_check = now
                if tx_rate < 0.1:
                    return {"status": "warning", "message": f"Baja tasa de transacciones: {tx_rate:.2f} tx/min"}
            except Exception:
                pass
            active_candidates = len(self.signal_logic.token_candidates) if self.signal_logic else 0
            if active_candidates == 0:
                return {"status": "warning", "message": "No hay tokens candidatos en monitoreo"}
            return {"status": "healthy", "message": f"Sistema funcionando correctamente. Tokens monitoreados: {active_candidates}, Tx rate: {tx_rate:.2f} tx/min"}
